...
import { GasCylinderOption, LocationOption, PriceTypeOption, PurchaseTypeOption, TransactionProps } from './types';
...
export default function TransactionEdit() {
    const { page, transaction, gasLocations, purchaseTypes, priceTypes } = usePage<UsePageProps>().props;
    ...
    const { data, setData, patch, processing, errors } = useForm<{
        ...
        purchase_type: transaction.purchase_type.value.toString(),
        price_type: transaction.price_type.value.toString(),
        quantity: transaction.quantity,
        unit_price: transaction.unit_price,
        total_price: transaction.total_price,
    });

    const [selectedDate, setSelectedDate] = useState<Date>(new Date(transaction.transaction_date));
    ...
    useEffect(() => {
        if (data.location_id && data.gas_cylinder_id) {
            fetch(`/transactions/stock-info?location_id=${data.location_id}&gas_cylinder_id=${data.gas_cylinder_id}`)
                .then((res) => res.json())
                .then((result) => {
                    // Add back the current transaction quantity to available stock
                    const adjustedStock = result.stock + transaction.quantity;
                    setMaxStock(adjustedStock || 0);
                    setAvailablePrices({ base: result.base_price || 0, retail: result.retail_price || 0 });
                })
                .catch(() => {
                    setMaxStock(transaction.quantity);
    ...
    const handleSubmit: FormEventHandler = (e) => {
        ...
        patch(route('transaction.update', transaction.id), {
            onError: () => {
    ...
    return (
        <>
            <Head title="Edit Transaksi">
                <meta name="description" content="Form edit transaksi kasir" />
            </Head>
...
TransactionEdit.layout = (page: ReactNode) => <AppLayout children={page} />;
