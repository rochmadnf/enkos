...
export default function ProductEdit() {
    ...
    } = usePage<{ product: { data: RowProps } }>().props;

    const form = useForm<FormValues>({
        name: product.name,
        capital_price: product.capital_price.price,
        selling_price: product.selling_price.price,
    });

    const updateSelectedProduct: FormEventHandler = (e) => {
        e.preventDefault();

        form.patch(route('product.update', { product_id: product.id }), {
            preserveScroll: true,
            replace: true,
            onError: () => {
                toast.error(`Terdapat data yang belum memenuhi syarat.`);
            },
            onSuccess: () => {
    ...
    return (
        <AppLayout pageHeader="Ubah Data Produk">
            ...
            <div className="flex items-center justify-center space-y-6 rounded-lg border border-slate-900/10 py-12">
                <form className="flex w-lg max-w-xl flex-col gap-y-4" onSubmit={updateSelectedProduct}>
