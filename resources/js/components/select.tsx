...
export const selectVariants = cva(
    cn(
        'flex w-full min-w-0 rounded-md border border-slate-900/25 bg-transparent px-3 py-2 text-base shadow-xs transition-[color,box-shadow] duration-300 outline-none selection:text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50 data-[placeholder]:text-muted-foreground',
...
function SelectTrigger({
    ...
    return (
        <SelectPrimitive.Trigger
            ...
            className={cn(
                selectVariants({ variant, isize, className }),
                "justify-between gap-2 whitespace-nowrap *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-muted-foreground",
            )}
            {...props}
        >
            {children}
